...
void Renderer2D::begin_scene(const OrthographicCamera& camera)
{
	...
	s_data.tri_vertex_buffer_ptr   = (TriVertex*)s_data.TRI_VERTICES;
	s_data.quad_vertex_buffer_ptr  = (QuadVertex*)s_data.QUAD_VERTICES;
	s_data.rhomb_vertex_buffer_ptr = (RhombVertex*)s_data.RHOMB_VERTICES;
...
void Renderer2D::end_scene()
{
	...
	size_t byte_size = (uint8_t*)s_data.tri_vertex_buffer_ptr - (uint8_t*)s_data.TRI_VERTICES;
	s_data.tri_vertex_buffer->push_data(s_data.TRI_VERTICES, byte_size);

	byte_size = (uint8_t*)s_data.quad_vertex_buffer_ptr - (uint8_t*)s_data.QUAD_VERTICES;
	s_data.quad_vertex_buffer->push_data(s_data.QUAD_VERTICES, byte_size);

	byte_size = (uint8_t*)s_data.rhomb_vertex_buffer_ptr - (uint8_t*)s_data.RHOMB_VERTICES;
...
void Renderer2D::submit(TriVertex* const buffer, uint32_t count, const std::shared_ptr<Texture2D>& texture)
{
	if (s_data.tri_vertex_count >= Renderer2DData::MAX_TRI_VERTICES)
	{
		RENDERER_INFO("Buffer size exceeded. Flushing before drawing next quad.");
	...
	{*/
	memcpy(s_data.tri_vertex_buffer_ptr, buffer, (size_t)count * sizeof(TriVertex));
	//}
